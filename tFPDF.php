...
function CellWithPadding($w, $h=0, $txt='', $border=0, $ln=0, $align='', $lpadding=0, $rpadding=0, $fill=false, $link='')
    {
        // Output a cell
        $txt = (string)$txt;
        $k = $this->k;
        $pd = $lpadding-$rpadding;
        if($this->y+$h>$this->PageBreakTrigger && !$this->InHeader && !$this->InFooter && $this->AcceptPageBreak())
        {
            // Automatic page break
            $x = $this->x;
        ...
        if($txt!=='')
        {
            if(!isset($this->CurrentFont))
                $this->Error('No font has been set');
            if($align=='R') {
                $dx = $w - $this->cMargin - $this->GetStringWidth($txt) + $pd;
            }
            elseif($align=='C')
                $dx = (($w-$this->GetStringWidth($txt))/2) + $pd;
            else
                $dx = $this->cMargin + $pd;
            if($this->ColorFlag)
                $s .= 'q '.$this->TextColor.' ';
...
function MultiCellWithOffset($w, $h, $txt, $border=0, $align='J', $loffset=0, $roffset=0, $fill=false)
{
    // Output text with automatic or explicit line breaks
    if(!isset($this->CurrentFont))
        $this->Error('No font has been set');
    ...
    while($i<$nb)
    {
        ...
        if($c=="\n")
        {
            ...
            if ($this->unifontSubset) {
//                $this->Cell($w,$h,mb_substr($s,$j,$i-$j,'UTF-8'),$b,2,$align,$fill);
                $this->CellWithPadding($w,$h,mb_substr($s,$j,$i-$j,'UTF-8'),$b,2,$align,$loffset,$roffset,$fill);
            }
            else {
//                $this->Cell($w,$h,substr($s,$j,$i-$j),$b,2,$align,$fill);
                $this->CellWithPadding($w,$h,substr($s,$j,$i-$j),$b,2,$align,$loffset,$roffset,$fill);
            }
            $i++;
            $sep = -1;
            $j = $i;
...
//                    $this->Cell($w,$h,mb_substr($s,$j,$i-$j,'UTF-8'),$b,2,$align,$fill);
            ...
            else
            {
                ...
                if ($this->unifontSubset) {
                    $this->CellWithPadding($w,$h,mb_substr($s,$j,$sep-$j,'UTF-8'),$b,2,$align,$loffset,$roffset,$fill);
                }
                else {
                    $this->CellWithPadding($w,$h,substr($s,$j,$sep-$j),$b,2,$align,$loffset,$roffset,$fill);
                }
                $i = $sep+1;
            }
            $sep = -1;
